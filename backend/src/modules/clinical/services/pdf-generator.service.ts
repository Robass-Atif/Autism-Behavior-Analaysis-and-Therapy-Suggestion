...
export class PdfGeneratorService {
  ...
  private addGoalsSection(doc: any, goalsData: any[]) {
    ...
    goalsData.forEach((goal, index) => {
      ...
      doc
        ...
        .fillColor("#666666")
        .text(
          `Category: ${goal.category} | Priority: ${goal.priority} | Status: ${goal.status}`,
        );
      ...
      // Background
      doc.rect(50, barY, barWidth, barHeight).fillColor("#e0e0e0").fill();

      // Progress
      doc.rect(50, barY, progressWidth, barHeight).fillColor("#4caf50").fill();
  ...
  private addSessionsSection(
    ...
    const analyzedCount = sessionsData.filter(
      (s) => s.status === "analyzed",
    ).length;
    ...
    if (includeTables) {
      ...
      sessionsData.slice(0, 10).forEach((session) => {
        ...
        doc
          ...
          .text(session.actionType || "N/A", col2, rowY, {
            width: 140,
            ellipsis: true,
          })
          .text(session.status, col3, rowY)
          .text(
            session.aiConfidence ? `${session.aiConfidence}%` : "N/A",
            col4,
            rowY,
          );
  ...
  private addFooter(doc: any, therapistName: string) {
    ...
    for (let i = 0; i < pages.count; i++) {
      ...
      doc.text(`Date: ${new Date().toLocaleDateString()}`, 50, 780, {
        align: "right",
      });
    }
  }
