...
export class AiAnalysisService {
  ...
  private async performAnalysisRequest(session: any, videoPath: string) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        this.logger.log(
          `🚀 FastAPI Request attempt ${attempt}/${maxRetries} for ${sessionId} at ${this.aiUrl}`,
        );
        ...
        this.logger.log(
          `✅ FastAPI response received for session ${sessionId} on attempt ${attempt}`,
        );
        ...
        if (data) {
          ...
          if (severity !== null) {
            if (severity >= 3) {
              ...
              recommendations.push(
                "Continue periodic developmental monitoring",
              );
            }
          }
          ...
          this.logger.log(
            `AI results updated for session ${sessionId} — Severity: ${severityLabel}, Confidence: ${session.aiConfidence}%`,
          );
        ...
        const errorMsg =
          ...
          "Unknown error";
        this.logger.error(
          `❌ FastAPI Request attempt ${attempt}/${maxRetries} failed for session ${sessionId}: ${errorMsg}`,
        );
    ...
    this.logger.error(
      `💀 FastAPI failed after ${maxRetries} attempts for session ${sessionId}`,
    );
