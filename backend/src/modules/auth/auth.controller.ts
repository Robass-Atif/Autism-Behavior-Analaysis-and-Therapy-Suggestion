...
export class AuthController {
  ...
  @ApiOperation({
    ...
    description:
      "User login with role derived from backend. Only ACTIVE accounts can login.",
  })
  @ApiResponse({
    status: 200,
    description: "Login successful (ACTIVE account)",
  })
  @ApiResponse({ status: 401, description: "Invalid credentials" })
  @ApiResponse({
    status: 403,
    description: "Account blocked (PENDING/REJECTED/SUSPENDED)",
  })
  ...
  @ApiOperation({
    ...
    description:
      "Prevents rejected therapists from reapplying after 3 attempts",
  })
  ...
  @ApiResponse({
    status: 403,
    description: "Email not eligible (rejected 3+ times)",
  })
  ...
  @ApiOperation({
    ...
    description:
      "Validates email verification token and activates the account or sets it to pending approval",
  })
  ...
  // 2️⃣3️⃣ POST /api/auth/resend-verification
