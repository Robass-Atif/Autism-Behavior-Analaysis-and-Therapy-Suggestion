...
import {
    Video, Clock, Calendar, CheckCircle,
    AlertCircle, Play, Trash2, Eye, Filter,
    Search, Grid, List, ChevronRight, Loader2
} from 'lucide-react';
...
export default function CaregiverVideoLibrary({ patientId, onRecordNew }: CaregiverVideoLibraryProps) {
            ...
            <div className="bg-white border border-zinc-200 p-8 font-mono">
                <div className="animate-pulse space-y-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        {[...Array(6)].map((_, i) => (
                            <div key={i} className="aspect-video bg-zinc-50 border border-zinc-100"></div>
                        ))}
                    </div>
                </div>
            </div>
    ...
    return (
        <div className="bg-white border border-zinc-200 font-mono text-zinc-900 selection:bg-zinc-100 relative overflow-hidden">
            {/* Accents */}
            <div className="absolute top-0 right-0 p-1"><div className="w-1 h-1 bg-zinc-200"></div></div>
            <div className="absolute bottom-0 left-0 p-1"><div className="w-1 h-1 bg-zinc-200"></div></div>
            ...
            <header className="p-8 border-b border-zinc-100">
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-10">
                    ...
                    {onRecordNew && (
                        ...
                        >
                            Initialize_New_Capture <Plus size={16} className="group-hover:rotate-90 transition-transform" />
                        </button>
                    )}
                </div>
                        ...
                        {filteredSessions.map((session: any) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="aspect-video bg-zinc-50 relative flex items-center justify-center border-b border-zinc-50">
                                        <div className="text-5xl opacity-[0.05] grayscale group-hover:opacity-20 transition-opacity">{actionInfo.icon}</div>
                                        ...
                                        <div className="absolute bottom-2 right-2 px-2 py-1 bg-zinc-900 text-white text-[8px] font-black tracking-widest">
                                        ...
                                        <div className="flex flex-col gap-3">
                                            ...
                                            {session.aiConfidence && (
                                                <div className="pt-3 border-t border-zinc-50 flex items-center justify-between">
                                                    ...
                                                    <span className="text-[10px] font-black text-zinc-900">{session.aiConfidence}%</span>
                                                </div>
                                            )}
                                        </div>
                                    </div>
                    ...
                    <div className="space-y-0 border-t border-zinc-100">
                        {filteredSessions.map((session: any) => {
                            ...
                            return (
                                ...
                                >
                                    <div className="w-12 h-12 bg-zinc-50 flex items-center justify-center text-3xl grayscale opacity-30 group-hover:opacity-100 transition-opacity border border-zinc-100">
                                        {actionInfo.icon}
                                    </div>
