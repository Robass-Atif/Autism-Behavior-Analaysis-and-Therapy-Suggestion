        ...
        <header className="border-b border-zinc-100 p-6 flex items-center justify-between">
          ...
          <div className="text-center">
            <h2 className="text-xs font-black uppercase tracking-[0.3em]">
              RECORDING INTERFACE V1
            </h2>
            <p className="text-[10px] text-zinc-400 font-bold uppercase tracking-widest mt-1">
              Select subject and action node
            </p>
          </div>
            ...
            <div className="lg:col-span-12 space-y-12">
              ...
              <section>
                <div className="flex items-center justify-between mb-6">
                  ...
                  <span className="text-[10px] font-bold text-zinc-300 tracking-tighter">
                    {GUIDED_ACTIONS.length} TOTAL NODES
                  </span>
                </div>
                  ...
                  {GUIDED_ACTIONS.map((action) => (
                    ...
                    >
                      <div className="flex flex-col gap-4">
                        ...
                        <div>
                          <span className="block text-xs font-black uppercase tracking-tight mb-1">
                            {action.name.replace("_", " ")}
                          </span>
                          <span
                            className={`block text-[9px] font-bold leading-relaxed opacity-60 line-clamp-2`}
                          >
      ...
      <div className="fixed inset-0 bg-white z-50 flex flex-col font-mono text-zinc-900">
        ...
        {error && (
          <div className="bg-red-50 p-4 border-l-4 border-red-600 flex items-center gap-4 mx-8 mt-4">
            ...
            <p className="text-[10px] font-black uppercase text-red-900 tracking-widest">
              {error}
            </p>
          </div>
        )}
      ...
      <div className="fixed inset-0 bg-white z-50 flex flex-col items-center justify-center p-8 text-center font-mono">
        ...
        <div className="max-w-md bg-zinc-50 border border-zinc-200 p-8 space-y-4 mb-12 relative">
          ...
          <p className="text-xs font-bold text-zinc-600 uppercase tracking-wide leading-relaxed">
            Node{" "}
            <span className="text-zinc-900">[{action.id.toUpperCase()}]</span>{" "}
            has been synchronized with clinical database.
          </p>
        ...
        <div className="flex flex-col gap-3">
          ...
          {isRecording && (
            <div className="flex gap-2">
              <div className="px-2 py-0.5 border border-emerald-500 bg-emerald-500/10 text-emerald-500 text-[8px] font-black uppercase tracking-widest">
                OPTIC STABLE
              </div>
              <div className="px-2 py-0.5 border border-emerald-500 bg-emerald-500/10 text-emerald-500 text-[8px] font-black uppercase tracking-widest">
                RAW SYNC
      ...
      <div className="flex-1 bg-zinc-950 relative overflow-hidden flex items-center justify-center">
        ...
        {isRecording && (
          <div className="absolute top-32 right-12 flex items-center gap-3 animate-pulse">
            ...
            <span className="text-red-600 font-black tracking-[0.4em] text-[10px]">
              RAW CAPTURE ACTIVE
            </span>
          </div>
        )}
        ...
        {error && (
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-red-950/90 border border-red-500 p-8 max-w-md w-full z-40">
            ...
            <p className="text-[10px] text-red-200/60 uppercase leading-loose">
              {error}
            </p>
            <button
              onClick={() => window.location.reload()}
              className="mt-6 w-full py-3 bg-red-600 text-white text-[10px] font-black uppercase tracking-widest"
            >
              FORCE REBOOT
            </button>
          </div>
                  ...
                  {action.instructions.map(
                    (instruction: string, idx: number) => (
                      <li key={idx} className="flex gap-4 items-start">
                        <span className="text-[10px] font-black bg-zinc-900 text-white w-5 h-5 flex items-center justify-center flex-shrink-0">
                          0{idx + 1}
                        </span>
                        <span className="text-[11px] font-bold uppercase leading-relaxed tracking-tight text-zinc-600">
                          {instruction}
                        </span>
                      </li>
                    ),
        ...
        <div className="flex flex-col md:flex-row items-center justify-between max-w-4xl mx-auto gap-8">
          <div className="flex flex-col items-center md:items-start gap-1 min-w-[120px]">
            ...
            <div className="flex items-baseline gap-2">
              <span
                className={`text-4xl font-black tracking-tighter ${timer >= action.duration ? "text-emerald-500" : "text-white"}`}
              >
                {formatTime(timer)}
              </span>
          ...
          >
            <div
              className={`w-20 h-20 border-2 transition-all duration-500 flex items-center justify-center ${
                isRecording
                  ? "border-red-600 bg-red-600/10"
                  : "border-white bg-transparent hover:border-zinc-400"
              }`}
            >
              <div
                className={`transition-all duration-300 ${
                  isRecording
                    ? "w-6 h-6 bg-red-600"
                    : "w-14 h-14 bg-red-600 border-4 border-zinc-900"
                }`}
              ></div>
            </div>
            {!isRecording && (
          ...
          <div className="flex gap-4">
            <button
              className="w-12 h-12 border border-white/10 flex items-center justify-center hover:bg-white/10 transition-all text-white/40 hover:text-white"
      ...
      <div className="h-6 bg-black border-t border-white/5 flex items-center overflow-hidden">
        <div className="flex gap-12 animate-marquee-slower whitespace-nowrap overflow-hidden pr-12">
          {Array.from({ length: 10 }).map((_, i) => (
            <span
              key={i}
              className="text-[8px] font-black text-white/10 uppercase tracking-[1em]"
            >
              CAPTURE MODE ACTIVE | SIGNAL ENCRYPTED | ABA-TS CORE LINK
            </span>
          ))}
        </div>
      </div>
...
const UploadReviewInterface = ({
  ...
  const handleUpload = () => {
    ...
    formData.append(
      "video",
      blob,
      `upload-${action.id}.${blob.type.includes("webm") ? "webm" : "mp4"}`,
    );
