...
import { RegisterTherapistData, useRegisterTherapist, useCheckRegistrationEligibility } from '../../../api/auth';
import toast from 'react-hot-toast';
import { Upload, X, Plus, Mail, Phone, MapPin, Briefcase, GraduationCap, FileText, Lock, CheckCircle2, AlertCircle, Loader2, Eye, EyeOff, ArrowRight } from 'lucide-react';
...
const therapistRegistrationSchema = z.object({
  ...
  licenseNumber: z.string().min(4, 'License number is required').max(50, 'License number must be less than 50 characters'),
...
export default function TherapistRegistrationScreen() {
  ...
  const onSubmit = async (data: TherapistFormInputs) => {
    ...
    if (emailCheckError) {
      toast.error('Please resolve the email eligibility issue before submitting.');
      return;
    }
  ...
  const nextStep = async () => {
    ...
    const fieldsForStep2 = ['licenseNumber', 'licenseType', 'issuingAuthority', 'licenseExpiryDate'];
    ...
    <div className="min-h-screen bg-white text-zinc-900 font-mono selection:bg-zinc-100 flex items-center justify-center p-4 md:p-8">
      <div className="w-full max-w-4xl">
        ...
        <div className="mb-8 flex items-end justify-between border-b border-zinc-100 pb-4">
          <div>
            <button onClick={() => navigate({ to: '/login' })} className="mb-4 text-xs font-bold text-zinc-400 hover:text-zinc-600 flex items-center gap-2 transition-colors uppercase tracking-widest">
              ...
              <div className="flex lg:flex-col lg:items-start items-center justify-between lg:justify-start gap-4 overflow-x-auto pb-4 lg:pb-0 scrollbar-hide">
                {[1, 2, 3, 4].map((step) => (
                  <div key={step} className="flex-shrink-0 group">
                    <div className={`flex items-center gap-3 transition-colors duration-300 ${step === currentStep ? 'text-zinc-900' :
                      step < currentStep ? 'text-zinc-400' : 'text-zinc-200'
                      }`}>
                      <div className={`
                        w-8 h-8 flex items-center justify-center text-xs font-bold border transition-all duration-300
                        ${step === currentStep ? 'bg-zinc-900 text-white border-zinc-900' :
                          step < currentStep ? 'bg-zinc-100 text-zinc-500 border-zinc-200' :
                  ...
                  <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-8">
                      <div className="md:col-span-2 group">
                        ...
                        />
                        {errors.fullName && <p className="mt-2 text-xs text-red-500 font-bold flex items-center gap-1"><AlertCircle size={10} /> {errors.fullName.message}</p>}
                      </div>
                      ...
                      <div className="group">
                        ...
                        {errors.professionalTitle && <p className="mt-2 text-xs text-red-500 font-bold flex items-center gap-1"><AlertCircle size={10} /> {errors.professionalTitle.message}</p>}
                      </div>
                      ...
                      <div className="group">
                        ...
                        <div className="relative">
                          <input
                            ...
                            onBlur={() => watchedEmail && checkEmailEligibility(watchedEmail)}
                          />
                        ...
                        {errors.email && <p className="mt-2 text-xs text-red-500 font-bold flex items-center gap-1"><AlertCircle size={10} /> {errors.email.message}</p>}
                      </div>
                      ...
                      <div className="md:col-span-2 group">
                        ...
                        />
                        {errors.phone && <p className="mt-2 text-xs text-red-500 font-bold flex items-center gap-1"><AlertCircle size={10} /> {errors.phone.message}</p>}
                      </div>
                    </div>
                  </div>
                )}
                  ...
                  <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-8">
                      <div className="group">
                        ...
                        />
                        {errors.licenseNumber && <p className="mt-2 text-xs text-red-500 font-bold flex items-center gap-1"><AlertCircle size={10} /> {errors.licenseNumber.message}</p>}
                      </div>
                      ...
                      <div className="group">
                        ...
                        {errors.licenseType && <p className="mt-2 text-xs text-red-500 font-bold flex items-center gap-1"><AlertCircle size={10} /> {errors.licenseType.message}</p>}
                      </div>
                      ...
                      <div className="group">
                        ...
                        />
                        {errors.issuingAuthority && <p className="mt-2 text-xs text-red-500 font-bold flex items-center gap-1"><AlertCircle size={10} /> {errors.issuingAuthority.message}</p>}
                      </div>
                      ...
                      <div className="group">
                        ...
                        <input
                          ...
                          className="w-full bg-zinc-50 border-b-2 border-zinc-200 px-4 py-3 text-sm focus:outline-none focus:border-zinc-900 focus:bg-white transition-all"
                        />
                        {errors.licenseExpiryDate && <p className="mt-2 text-xs text-red-500 font-bold flex items-center gap-1"><AlertCircle size={10} /> {errors.licenseExpiryDate.message}</p>}
                      </div>

                      <div className="md:col-span-2">
                        <label className="block text-xs font-bold uppercase text-zinc-500 mb-2">License Document</label>
                        <div className="border border-dashed border-zinc-300 p-8 text-center bg-zinc-50 hover:bg-zinc-100 hover:border-zinc-400 transition-all cursor-pointer group relative">
                          ...
                          {!licenseFile ? (
                            <div className="flex flex-col items-center gap-2">
                              <Upload className="text-zinc-400 group-hover:text-zinc-600 transition-colors" size={24} />
                              <p className="text-sm font-bold text-zinc-600">CLICK_TO_UPLOAD</p>
                              <p className="text-xs text-zinc-400 font-mono">PDF/JPG_MAX_5MB</p>
                            </div>
                          ) : (
                            <div className="flex items-center justify-between bg-white border border-zinc-200 p-3 w-full">
                              <div className="flex items-center gap-3">
                                ...
                                <div className="text-left">
                                  <p className="text-xs font-bold text-zinc-900 truncate max-w-[200px]">{licenseFile.name}</p>
                                  <p className="text-[10px] text-zinc-400 font-mono">{(licenseFile.size / 1024).toFixed(1)} KB</p>
                                </div>
                              </div>
                              <button
                                type="button"
                  ...
                  <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
                    ...
                    <div className="space-y-6">
                      <div className="group">
                        ...
                        />
                        {errors.organizationName && <p className="mt-2 text-xs text-red-500 font-bold flex items-center gap-1"><AlertCircle size={10} /> {errors.organizationName.message}</p>}
                      </div>
                      ...
                      <div className="group">
                        ...
                        />
                        {errors.workAddress && <p className="mt-2 text-xs text-red-500 font-bold flex items-center gap-1"><AlertCircle size={10} /> {errors.workAddress.message}</p>}
                      </div>
                    </div>
                  </div>
                )}
                  ...
                  <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
                    ...
                    <div className="space-y-6">
                      <div className="group">
                        ...
                        {errors.password && <p className="mt-2 text-xs text-red-500 font-bold flex items-center gap-1"><AlertCircle size={10} /> {errors.password.message}</p>}

                        <div className="mt-3 grid grid-cols-1 md:grid-cols-3 gap-2">
                          <div className={`text-[10px] uppercase font-bold px-2 py-1 border ${watchedPassword?.length >= 8 ? 'bg-emerald-50 text-emerald-600 border-emerald-200' : 'bg-zinc-50 text-zinc-400 border-zinc-200'}`}>
                            Min 8 Chars
                          </div>
                          <div className={`text-[10px] uppercase font-bold px-2 py-1 border ${/[A-Z]/.test(watchedPassword || '') ? 'bg-emerald-50 text-emerald-600 border-emerald-200' : 'bg-zinc-50 text-zinc-400 border-zinc-200'}`}>
                            1 Uppercase
                          </div>
                          <div className={`text-[10px] uppercase font-bold px-2 py-1 border ${/[0-9]/.test(watchedPassword || '') ? 'bg-emerald-50 text-emerald-600 border-emerald-200' : 'bg-zinc-50 text-zinc-400 border-zinc-200'}`}>
                            1 Number
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="group">
                        ...
                        {errors.confirmPassword && <p className="mt-2 text-xs text-red-500 font-bold flex items-center gap-1"><AlertCircle size={10} /> {errors.confirmPassword.message}</p>}
                      </div>
                      ...
                      <div className="pt-6 border-t border-zinc-100 space-y-4">
                        <label className="flex items-start gap-4 cursor-pointer group">
                          ...
                          <span className="text-xs text-zinc-500 group-hover:text-zinc-700 font-medium">
                            I accept the <a href="#" className="text-zinc-900 underline decoration-zinc-300 underline-offset-2 hover:decoration-zinc-900">Terms of Service</a>
                          </span>
                        </label>
                        {errors.agreeToTerms && <p className="ml-9 -mt-3 text-xs text-red-500 font-bold">{errors.agreeToTerms.message}</p>}
                        ...
                        <label className="flex items-start gap-4 cursor-pointer group">
                          ...
                          <span className="text-xs text-zinc-500 group-hover:text-zinc-700 font-medium">
                            I acknowledge <strong className="text-zinc-900">HIPAA Compliance</strong> requirements
                          </span>
                        </label>
                        {errors.agreeToHIPAA && <p className="ml-9 -mt-3 text-xs text-red-500 font-bold">{errors.agreeToHIPAA.message}</p>}
                        ...
                        <label className="flex items-start gap-4 cursor-pointer group">
                          ...
                          <span className="text-xs text-zinc-500 group-hover:text-zinc-700 font-medium">
                            I accept the <a href="#" className="text-zinc-900 underline decoration-zinc-300 underline-offset-2 hover:decoration-zinc-900">Privacy Policy</a>
                          </span>
                        </label>
                        {errors.agreeToPrivacy && <p className="ml-9 -mt-3 text-xs text-red-500 font-bold">{errors.agreeToPrivacy.message}</p>}
                      </div>
                    </div>
                  </div>
                )}
                ...
                <div className="mt-10 pt-6 border-t border-zinc-100 flex items-center justify-between">
                  {currentStep > 1 ? (
                    <button
                      type="button"
                      onClick={prevStep}
                      className="px-6 py-3 text-xs font-bold uppercase tracking-widest text-zinc-500 hover:text-zinc-900 hover:bg-zinc-50 transition-all"
                    >
                      &larr; Back_Step
                    </button>
                  ) : (
                  ...
                  {currentStep < totalSteps ? (
                    ...
                    >
                      <span className="mr-2">Next_Step</span>
                    ...
                    <button
                      ...
                      className="group relative inline-flex items-center justify-center px-8 py-3 bg-zinc-900 text-white text-xs font-bold uppercase tracking-widest hover:bg-zinc-800 transition-all active:scale-[0.98]"
                    >
                      <span className="mr-2">Complete_Registration</span>
