...
// Caregiver Schedule Module
export const CAREGIVER_SCHEDULE_ENDPOINTS = {
  LIST: '/caregiver-schedule',
  CREATE: '/caregiver-schedule',
  UPDATE: (id: string) => `/caregiver-schedule/${id}`,
  DELETE: (id: string) => `/caregiver-schedule/${id}`,
}
...
export const getFileUrl = (path?: string): string => {
  ...
  // Intercept video file requests to route through our secure decrypted stream API
  if (path.startsWith('/uploads/videos/')) {
    const token = localStorage.getItem('token') || '';
    return `${API_BASE_URL}/clinical${path}?token=${token}`
  }
